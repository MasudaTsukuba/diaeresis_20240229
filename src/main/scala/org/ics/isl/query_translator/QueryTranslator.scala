...
object QueryTranslator {
    ...
    def main(args: Array[String]): Unit = {
      ...
      //todo: subPartIndex
       ...
       //todo: end subPartIndex
        ...
        println(inputPath)  //2023/8/8
        var j = 0       
        val fileList = listFiles(new File(inputPath), recursive = false)
        nodeIndex.cache //[(uri_instance, class)]
                    
        fileList.foreach(queryFile => {
            println("=================")  //2024/2/29
            println(queryFile)  // 2024/2/29
            ...
            println("Variables in Query: ", variables.size)  //2023/8/8
            variables.foreach (var_element =>
              {
                println(var_element)
              }
            )
            ...
            println("Triple Patterns in Query: ", triplePatterns.size)  //2023/8/8
            triplePatterns.foreach(triplePattern =>
            {
              println(triplePattern)
            }
            )
            ...
            println("queryMappings =============") //2024/3/8
            println(queryMappings)
    ...
    def buildQueryIndex(queryMappings: Map[Int, String], triplePatterns: Array[(Int, (String, String, String))],
        ...
        println("In buildQueryIndex.")  // 2024/2/29
        println(queryMappings)  // 2024/2/29
//        classIndex.foreach{case(str0, array0) =>
//            println(str0)
//            array0.foreach(str1 => {println(str1)})
//        }  // 2024/2/29
        ...
        val queryIndex = queryMappings.map{case(tpId, uri) => { //(id -> uri) -- constant: uri_instance, variable(if exist in query Class): uri_Class (?x rdf:type Class)
            ...
            println("tpId=", tpId, "  c=", c)  //2024/2/29
        ...
        if(queryIndex.contains(-1))
            ...
            return mapSubPartitions(queryIndex, queryMappings, triplePatterns, classIndex, subPartIndex, nodeIndex) //TODO: add partition if predicate is a variable based on NEW index (s/o, partition) ++ small partition!
    }
