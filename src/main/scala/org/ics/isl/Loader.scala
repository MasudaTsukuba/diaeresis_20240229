...
object Loader {
    ...
    def loadInstanceGraph(spark: SparkSession, dataset: String, hdfs: String): Graph[(String, Seq[String], Seq[(String, String)]), String] = {
        ...
        val ePartNum = edges.partitions.length
        println("ePartNum " + ePartNum)
        val vPartNum = vertices.partitions.length
        println("vPartNum " + vPartNum)
    ...
    def loadSchemaImportance(dataset: String): Map[Long, Double] = {
        println("In loadSchemaImportance ===================") // 2024/3/6
