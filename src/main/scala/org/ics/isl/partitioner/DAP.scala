...
object DAP {
	...
	def main(args: Array[String]): Unit = {
		...
		if (args.length != 5) {
			...
			println("-->Arguments: dataset name, number of partitions, hdfs base path, instances path, schema path")
			System.exit(-1)
		}

		dataset = args(0)
		numOfPartitions = args(1).toInt
		hdfs = args(2)
		...
		val www = schemaGraph.edges.collect()
		println("schemaGraph.edges ======== " + www.length)
		println(www(0))
		println(www(1))
		println(www(2))
		val uuu = schemaGraph.vertices.collect()
		println("schemaGraph.vertices ======== " + uuu.length)
		println(uuu(0))
		println(uuu(1))
		println(uuu(2))
		val vvv = schemaGraph.triplets.collect()
		println("schemaGraph.triplets ======== " + vvv.length)
		println(vvv(0))
		println(vvv(1))
		println(vvv(2))
		...
		val iii = instanceGraph.edges.collect()
		println("instanceGraph.edges ======== " + iii.length)
		println(iii(0))
		println(iii(1))
		println(iii(2))
		val jjj = instanceGraph.vertices.collect()
		println("instanceGraph.vertices ======== " + jjj.length)
		println(jjj(0))
		println(jjj(1))
		println(jjj(2))
		val kkk = instanceGraph.triplets.collect()
		println("instanceGraph.triplets ======== " + kkk.length)
		println(kkk(0))
		println(kkk(1))
		println(kkk(2))


		Preprocessor.computeMeasures(spark, sc, instanceGraph, schemaGraph, dataset, hdfs)
		val instanceCount = sc.textFile(instancePath).count
		println("instanceCount: " + instanceCount) // 2024/3/6
		...
		val timeDAP = System.currentTimeMillis - timeStart
		println("DAP - partitioning time: " + timeDAP + "ms")
		spark.stop()
	}

	private def computePartitionStatistics(spark: SparkSession): Unit = {
	...
	private def clusterSchema(spark: SparkSession, instanceCount: Long, dataset: String, hdfs: String): Unit = {
		println("In DAP: clusterSchema ===============================")  // 2024/3/6
		val schemaGraph = Loader.loadSchemaGraph(spark, dataset, hdfs).cache
		...
		if (!HdfsUtils.hdfsExists(hdfs + dataset + Constants.weightedVertices + "_" + numOfPartitions) && !HdfsUtils.hdfsExists(hdfs + dataset + Constants.weightedEdges + "_" + numOfPartitions)) {
			...
			Preprocessor.saveWeightedGraph(weightedVertices, weightedEdges, numOfPartitions, dataset, hdfs)
		}
		else {
			weightedSchema = Loader.loadWeightedGraph(spark.sparkContext, numOfPartitions, dataset, hdfs)
		}

		val nonCentroids = schemaVertices.keys.toSeq //take id of vertices -- all vertices

		val result = if (!HdfsUtils.hdfsExists(hdfs + dataset + Constants.shortestPaths + "_" + numOfPartitions)) {
			println("ShortestPaths DO NOT exist!")
			// weighted shortest paths for every node to every centroid
...
//			Preprocessor.saveShortestPathVertices(result, numOfPartitions, dataset, hdfs)
//		val result = Loader.loadShortestPaths(spark.sparkContext, numOfPartitions, dataset, hdfs)

		val diffs = result.filter(node => node._2.nonEmpty)
			.flatMap(node => dependenceDiff(importanceMap, node._2))
		println("diffs ======== "+diffs.count())  // 2024/3/8
		val xxx = diffs.collect()  // 2024/3/8
		println(xxx(0))  // 2024/3/8
		println(xxx(1))  // 2024/3/8

		val maxDiff = diffs.max
		val minDiff = diffs.min
		println("maxDiff = "+maxDiff+"  minDiff = "+minDiff)  // 2024/3/6

		//result: case(node_centroid_id, dependenceMap)
		//dependence() return: [CentroidId, Dependence]
		//node._1: node_id (no centroid!)
		//node._2: (centroid_id, [dependence_path_value, Seq(id_dest, id_source, namePredicate)]) == Dependence(?)

		val schemaCluster = result.filter(node => node._2.nonEmpty)
		...
		val clusterTriples = createClusters(schemaCluster, schemaVertices, instanceCount, dataset, hdfs)  // return value?
	...
	private def createClassIndex(schemaCluster: Map[Long, Array[String]], centroids: Seq[Long], dataset: String): Unit = {
	...
	private def createClusters(dependencies: Seq[(Long, Map[Long, (Double, Seq[(Long, Long, String)])])],
							   schemaVertices: Map[Long, String],
							   instanceCount: Long,
							   dataset: String, hdfs: String): Map[Long, Array[(Long, Long, String)]] = {
		println("In DAP: createClusters ===== ")  // 2024/3/6
		var clusterCounter = Map[Long, Long]()
		var clusterNodes = Map[Long, Array[Long]]()
		var clusterTriples = Map[Long, Array[(Long, Long, String)]]()

		//until now: (node_id, DepMap) --dependencies: (cId_centroid, (cId2, (dep, path))) -- path: (Long, Long, String)
		val centroidMap = aggregateCentroids(dependencies) ////now: (cID, DepMap) --  DepMap: (ordering) Array[(dep, node_id, Seq(path)), ..]  // centroidMap.map{case(cId, dependencies) => (cId, dependencies.sortBy(_._1)(Ordering[Double].reverse))}
		val schemaNodeCount: Map[String, Long] = Loader.loadSchemaNodeCount(dataset)
		...
		println(">> threshold: " + threshold +" >>instanceCount: "+instanceCount)

		// val test = centroidMap.map{case(cID, dependencies) => (cID, dependencies.map(d => d._2) )}
		// test.foreach{case(cID, nodes) => println("-- centroid- "+cID +": "+ nodes.mkString(","))}

		// val test1 = dependencies.map{case(node, dep) => (node, dep.map(d => (d._1)))}
		//test1.foreach{case(node, cids) => println("~~" +node+ ": "+cids.mkString(","))}

		//Dependencies : centroidMap.foreach{case(cID, dependencies) => {dependencies.foreach(d => println(cID + " ~ "+d._2+": "+d._1) )}}

		//todo: val test = centroidMap.map{case(cID, dependencies) => (cID, dependencies.map(d => d._2) )}
		//test.foreach{case(cID, nodes) => println("-- "+cID +": "+ nodes.mkString(","))}

		val numNodes = centroidMap.map { case (cID, dependencies) => dependencies.length }.min
		val numNodesMax = centroidMap.map { case (cID, dependencies) => dependencies.length }.max
		//println("SIZE max : "+numNodesMax+ " - min:" + numNodes)

		//centroidMap.foreach{case(cID, dependencies) => println(cID + ">>>  ")}
		//initialize maps
		centroids.foreach(c => {
			clusterCounter = clusterCounter + (c -> 0.toLong)
...
//		val array = Array("<http://www.w3.org/2004/02/skos/core#Concept>", "<http://www.w3.org/2002/07/owl#Class>", "<Version 3.8>", "<http://dbpedia.org/ontology/>")
		val array = Array()  // 2024/3/11
		val initialValidNodes = schemaVertices.keys.toSet.filter(x => !centroids.contains(x) && !array.contains(x)) //all nodes except centroids and invalid
		var leftoutNodes = initialValidNodes //all nodes except from centroids
		var leftoutSize = leftoutNodes.map(schemaVertices(_)).map(schemaNodeCount.getOrElse(_, 0.toLong)).sum

		println("numNodes = "+numNodes)  // 2024/3/6
		println("leftoutSize = "+leftoutSize)  // 2024/3/6
		while (leftoutSize != 0) {
			for {i <- 0 until numNodes if leftoutSize != 0} {
				println("i = "+i)  // 2024/3/6
				centroidMap.foreach { case (cId, dependencies) => //dependencies: (cId_centroid, (cId2, (dep, path)))
					val node = dependencies(i)._2
					val path = dependencies(i)._3
					println("cId = "+cId+ "  node = "+node+"  path = "+path) //2024/3/6
					breakable {
						//if node is clustered continue
						if (clusteredNodes.contains(node) || array.contains(node))
							break
						//todo: if currCentroid is empty add the centroid node in  - none centroid empty
						//current centroids nodes
						...
						val newNodesize = if (newNodes.nonEmpty) { //estimate the sum of instances of the added nodes
							newNodes.map(n => schemaNodeCount.getOrElse(schemaVertices(n), 0.toLong))
								.reduce(_ + _)
						}
						...
						println("newNodesize = "+newNodesize)  // 2024/3/6

						val curClusterSize = clusterCounter(cId)
						val num = curClusterSize + newNodesize < threshold

						//if(curClusterSize < 0 )
						// println("Negative_"+cId +": " + clusterCounter(cId))

						//println("**NODE: "+node+" ***")
						//newNodes.foreach(println(_))
						//println("###### " + cId + " curr: " + curClusterSize + " newNodesize: "+newNodesize + " -"+num)
					...
					//!! println("** "+cId + " cluster size: "+clusterCounter(cId) + "  - "+clusterNodes(cId).length)
				}  // end of centroidMap.foreach {

				leftoutNodes = leftoutNodes -- clusteredNodes.toSet

				if (leftoutSize < 2) { // 2024/3/11
					for {xxx <- leftoutNodes} { // 2024/3/11
						println(xxx)
					}
				}

				leftoutSize = if (leftoutNodes.nonEmpty) {
						leftoutNodes.map(schemaVertices(_)).map(schemaNodeCount.getOrElse(_, 0.toLong)).reduce(_ + _)
					}
					else {
						0
					}
			...
			println("thresholdError = "+thresholdError)  // 2024/3/6
//			break  // debug // 2024/3/6
		...
		val updatedClusterClasses = clusterClasses.map { case (cId, schemaNodes) => if (schemaNodes.length == 0) (cId, schemaNodes :+ schemaVertices(cId)) else (cId, schemaNodes) }
	...
	private def aggregateCentroids(dependencies: Seq[(Long, Map[Long, (Double, Seq[(Long, Long, String)])])]):
	Map[Long, Array[(Double, Long, Seq[(Long, Long, String)])]] = {
		var centroidMap = Map[Long, Array[(Double, Long, Seq[(Long, Long, String)])]]()
		dependencies.foreach { case (node, depMap) => //dependencies: (node, (cId2, (dep, path)))
		...
		centroidMap.map { case (cId, dependencies) => (cId, dependencies.sortBy(_._1)(Ordering[Double].reverse)) } //now: (cID, DepMap) --  DepMap: (dep, node_id, Seq(path))
	...
	private def partitionClusters(spark: SparkSession, finalClusters: RDD[((String, String, String), String)], dataset: String, hdfs: String): Unit = {
		import spark.implicits._
		//val centroidMap = Loader.loadCentroidMap(numOfPartitions)

		finalClusters.map(x => (x._2, x._1)) //[(centroid_id, triple)]
			.partitionBy(new CustomPartitioner(numOfPartitions)) //take first element of each tuple - centroid_ID
			.map(x => x._2) //x._2:triple
			.toDF().write.mode("overwrite").format("parquet").save(hdfs + dataset + Constants.clusters + "_" + numOfPartitions + "_bal") //create files for each centroid and write triples belong to it.
		...
		stats.foreach { case (partition, size) =>
			if (partition.length == 1)
				sw.write("0" + partition + " " + size + "\n")
			else
				sw.write(partition + " " + size + "\n")
		}
		sw.close()
	}

	//create folders from the files in the previous step.
	private def subPartitionClusters(spark: SparkSession, dataset: String, hdfs: String, partitionNum: Int): Unit = {
		import spark.implicits._
		...
		nodeIndexDf.write.mode("overwrite").format("parquet").save(hdfs + dataset + Constants.nodeIndexFile + "_" + numOfPartitions)
		...
		partitions.foreach(p => {
			...
			partition.rdd.repartition(roundUp(partSize / 7.0)) //partition.rdd.repartition(roundUp(partSize/7.0))
				.filter { case (s, o, p) => p.length < 150 }
				.toDF.as[(String, String, String)]
				.map { case (s, o, p) => (s, o, uriToStorage(p)) }
				.write.mode("overwrite").partitionBy("_3").parquet(basePath + "/" + partitionName) //create folders for each predicate inside to each folder of cluster
		...
		partitions.foreach(p => { //p: part-00000-4dd825ad-46db-43a8-9ea9-09092757d2aa-c000.snappy.parquet  -- SWDF_data/cluster/clusters_8_bal/00000
		   ...
		   */
			...
			//todo: more predicates are connected with classes
			...
			uris.write.mode("overwrite").format("parquet").save(hdfs + dataset + Constants.classSubPartIndexFile + "_" + numOfPartitions + "__" + partitionName)
	...
	def uriToStorage(uri: String): String = {
		uri.replaceAll("[^A-Za-z0-9]", "_").toLowerCase
	}


	def fillStr(str: String): String = {
		List.fill(5 - str.length)("0").mkString + str
	}
	...
	def generateNodeIndex(spark: SparkSession, dataset: String, hdfs: String): Unit = {
		import spark.implicits._
		val fullPath = hdfs + dataset + Constants.clusters + "_" + numOfPartitions + "_bal/" //files for partitions created by partitionClusters()
		val cluster = Loader.loadClusters(spark, fullPath).distinct //load all partitioned files? .parquet?
		val nodeIndex = cluster.rdd.filter { case (s, o, p) => p == Constants.RDF_TYPE }
	 ...
	 */
	private class CustomPartitioner(override val numPartitions: Int) extends Partitioner {
	...
	private def partitionInstances(
		...
		// val centroidMap = Loader.loadCentroidMap(numOfPartitions, dataset)//.map(x => (x._2, x._1))
		...
		val partitionedInstances = instanceGraph.triplets.filter(t => t.srcAttr._2.nonEmpty || t.dstAttr._2.nonEmpty)
		...
		val typeTriples = instanceGraph.triplets.filter(t => t.srcAttr._2.nonEmpty || t.dstAttr._2.nonEmpty)
		...
		val partitionTypeTriples = typeTriples.map(t => (t, brIndex.value.getOrElse(t._3, Array()))) // [(src, RDF:TYPE, Class_URI), (id_centroid() ) ]
			.filter(!_._2.isEmpty)
		...
		val labelTriples = instanceGraph.triplets.filter(t => (t.srcAttr._3.nonEmpty && t.srcAttr._2.nonEmpty) || (t.dstAttr._3.nonEmpty && t.dstAttr._2.nonEmpty)) //t.srcAttr._3: case(p, o)
	...
	private def populateLabels(srcAttr: (String, Seq[String], Seq[(String, String)])): Seq[((String, String, String), Seq[String])] = {
		val uri = srcAttr._1
	 ...
	 */
	private def rdfCluster(spark: SparkSession, //edge: (s, o, p) vertices: (id, (uri, types, labels))
	 ...
	 */
	private def createWeightedGraph(spark: SparkSession,
		...
		//cc.take(10).foreach(x=> println(x._1._1+"-"+x._1._2+"-"+x._1._3+": "+x._2._1 +"--"+x._2._1))
	 ...
	 */
	private def dependenceDiff(importanceMap: Map[Long, Double],
							   centroidMap: Map[Long, (Double, Seq[(Long, Long, String)])]): Iterable[Double] = {
//		println("In DAP: dependenceDiff =====")
	 ...
	 * used of calculation of max and min dependence. (Normalization)
	 */
	private def calculateDependenceDiff(srcBC: Double, centroidTuple: (Double, Seq[(Long, Long, String)])) = {
	 ...
	 * centroidMap: [ centroidID, ( Sum(BC,CC) ,path to each centroid) ]
	 * returns: Map[CentroidId, Dependence]
	 */
	// map(node =>(node._1, dependence(importanceMap, node._2, maxDiff, minDiff)))
	private def dependence(importanceMap: Map[Long, Double],
						   centroidMap: Map[Long, (Double, Seq[(Long, Long, String)])],
	 ...
	 * returns: Dependence
	 */
	private def calculateDependence(srcBC: Double, centroidTuple: (Double, Seq[(Long, Long, String)]), max: Double, min: Double) = {
		...
		(1 / pathSizeSquared) * normDiff
	}

	/**
	 * Helper function to map triple to centroid Id (used for flattening)
	 */
	def mapIdToTriples(centroidTuple: (Long, (Double, Seq[(Long, Long, String)]))): Seq[(VertexId, (VertexId, VertexId, String))] = {
	 ...
	 */
	private def cardinalityCloseness(triplet: EdgeTriplet[String, (String, Double)], c: Long, cc: Map[(String, String, String), (Long, Long)]): Double = {
		val t1 = (triplet.srcAttr, triplet.attr._1, triplet.dstAttr)
		val t = if (cc.keySet.contains(t1)) t1 else (t1._3, t1._2, t1._1)
		val cardinalities = cc.getOrElse(t, (1.toLong, 0.toLong))
	 ...
	 * weight: (destBC/CC)
	 */
	private def addWeight(c: Long, triplet: EdgeTriplet[String, (String, Double)], importance: Map[Long, Double], cc: Map[(String, String, String), (Long, Long)]): Edge[(String, Double)] = {
		val destImportance = importance(triplet.dstId)
		val curCC = cardinalityCloseness(triplet, c, cc)
		val newEdge = new Edge[(String, Double)]
		newEdge.attr = (triplet.attr._1, destImportance / curCC) //based paper
		//!!todo based our idea: newEdge.attr = (triplet.attr._1, destImportance * curCC)
	 ...
	 */
	def normalizeValue(value: Double, min: Double, max: Double): Double = (value - min) / (max - min)

	/**
	 * finds top k centroids based on BC
	 */
	private def findCentroids(importance: Map[Long, Double], schemaMap: Map[Long, String], k: Int) = {
		println("In DAP: findCentroids ======")
